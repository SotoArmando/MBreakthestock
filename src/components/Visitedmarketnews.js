...
export default function Visitedmarketnews({ category, datetime, headline, related, source, image, summary, url, basis }) {
    ...
    return <div className="col corebox_17  center items_start  border_t4 pad_b30 pad_t30 mobilepad_l24 pad_l24 btn " style={{ flexBasis: basis + 'px', maxWidth: basis + 'px' }}>
        <a className="allsize col   space_between fore_11 pad_r15 hover_b" target='_tab' href={url}>
            ...
            <div className="col    pad_b23 ">
                ...
                <div className="row center space_between allwidth mar_t19">
                    <span className="f_1 f500 fore_16  "><span className="fore_green f600">{source} {new Date(datetime * 1000).toLocaleDateString()}</span> <span className="f_1 ls_27 f500" >{headline}</span> <br/> <span className="f_0 ls_27 fore_14 f400">{summary}</span>.</span>
                </div>
            </div>
